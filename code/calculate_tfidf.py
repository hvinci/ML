...
def calculate_tfidf(input_folder, output_folder):
    # 如果输出文件夹不存在，创建它
            ...
            with open(output_file_path, 'w', encoding='utf-8') as output_file:
                for file_path, doc_tfidf_dict in tfidf_dict.items():
                    for feature, tfidf in doc_tfidf_dict.items():
                        # 格式化 TF-IDF 值为两位小数
                        formatted_tfidf = f"{tfidf:.2f}"
                        output_file.write(f"{feature}:{formatted_tfidf}\n")
                    output_file.write("\n")

if __name__ == "__main__":
    # 定义输入文件夹（过滤后的名词数据）
    input_folder = "code/cnews/filtered_noun_data"

    # 定义输出文件夹
    output_folder = "code/cnews/tfidf_values"

    # 执行计算 TF-IDF 操作
    calculate_tfidf(input_folder, output_folder)
